...
public class PlayerInventory : NetworkBehaviour
{
    ...
    {
        // 1. Instantiate the prefab as usual
        GameObject thrown = Instantiate(GetSelectedItem(slot).prefab, spawnPos,
                                        Quaternion.LookRotation(direction));

        // 2. Mirror uses NetworkServer.Spawn() instead of a component method
        // Make sure 'thrown' has a NetworkIdentity component attached
        NetworkServer.Spawn(thrown);

        // 3. Call your local logic as before
